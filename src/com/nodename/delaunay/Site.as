package com.nodename.delaunay {
    ...
    public final class Site implements ICoord {
        ...
        public static function create(p:Point, index:int, weight:Number):Site {
            if (_pool.length > 0) {
                return _pool.pop().init(p, index, weight);
            } else {
                return new Site(PrivateConstructorEnforcer, p, index, weight);
            }
        }
        ...
        public function Site(lock:Class, p:Point, index:int, weight:Number) {
        ...
        private function init(p:Point, index:int, weight:Number):Site {
            _coord = p;
            _siteIndex = index;
            this.weight = weight;
            this.color = color;
