 ...
 * is included in list copies of any software which is or includes a copy
 * or modification of this software and in list copies of the supporting
...
package com.nodename.delaunay {
    ...
    public final class Voronoi {
        ...
        public function Voronoi(points:Vector.<Point>, plotBounds:Rectangle) {
            _sites = new SiteList();
        ...
        private function addSites(points:Vector.<Point>):void {
        ...
        private function addSite(p:Point, index:int):void {
            var weight:Number = Math.random() * 100;
            var site:Site = Site.create(p, index, weight);
            _sites.push(site);
        ...
        public function voronoiBoundaryForSite(coord:Point):Vector.<Segment> {
            return visibleLineSegments(selectEdgesForSitePoint(coord, _edges));
        }

        public function delaunayLinesForSite(coord:Point):Vector.<Segment> {
            return delaunayLinesForEdges(selectEdgesForSitePoint(coord, _edges));
        }

        public function voronoiDiagram():Vector.<Segment> {
            return visibleLineSegments(_edges);
        }

        public function delaunayTriangulation(keepOutMask:BitmapData = null):Vector.<Segment> {
            return delaunayLinesForEdges(selectNonIntersectingEdges(keepOutMask, _edges));
        }

        public function hull():Vector.<Segment> {
        ...
        public function hullPointsInOrder():Vector.<Point> {
            var myHullEdges:Vector.<Edge> = hullEdges();

            var points:Vector.<Point> = new Vector.<Point>();
            if (myHullEdges.length == 0) {
                return points;
            }

            var reorderer:EdgeReorderer = new EdgeReorderer(myHullEdges, Site);
            myHullEdges = reorderer.edges;
            var orientations:Vector.<LR> = reorderer.edgeOrientations;
            reorderer.dispose();

            var orientation:LR;

            var n:int = myHullEdges.length;
            for (var i:int = 0; i < n; ++i) {
                var edge:Edge = myHullEdges[i];
        ...
        public function spanningTree(myType:String = "minimum", keepOutMask:BitmapData = null):Vector.<Segment> {
            var edges:Vector.<Edge> = selectNonIntersectingEdges(keepOutMask, _edges);
            var segments:Vector.<Segment> = delaunayLinesForEdges(edges);
            return kruskal(segments, myType);
        }
